...
func CreateValidTestEmailVerificationTokens(
	...
	for i := 0; i < 2; i++ {
		if tokenString, err := utils.GenerateSlug(80); err != nil {
			...
			validEmailTokens = append(validEmailTokens, models.EmailVerificationToken{
				UserSlug:  user.Slug,
				...
				ExpiresAt: now.Add(time.Duration(9) * time.Minute),
			})
	...
	if result := dbs.ApiGateway.Create(&validEmailTokens); result.Error != nil {
